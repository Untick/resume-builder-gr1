...
from chatGPT.getGPT import about_gpt, experience_gpt
...
def create_cv(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'order':
            ...
            data_about = {'fullname': cvo.fullname, 'position_sought': cvo.position_sought,
                          'about_me': cvo.about_me, 'experience_level': cvo.experience_level, }
            data_exp = {'last_job': cvo.last_job, 'last_position': cvo.last_position,
                        'key_skills': cvo.key_skills, 'experience_level': cvo.experience_level, }
        if action == 'result':
            ...
            cv_data = {
                ...
                'employment_type': request.POST.get('employment_type', ''),
                'schedule': request.POST.get('schedule', ''),
                'experience': experience_gpt(data_exp),
                'about_me': about_gpt(data_about),
                'key_skills': request.POST.get('key_skills', ''),
                'education_university': request.POST.get('education_university', ''),
            }
...
def update_cv(request, cv_id):
    ...
    if request.method == 'POST':
        ...
        cv.experience = request.POST.get('experience', cv.experience),
